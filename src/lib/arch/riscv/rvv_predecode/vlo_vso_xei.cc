...
rvv_insn_desc rvv_ldst_oindexed_predecode(const uint32_t insn) {
  ...
  std::string mnemonic = fmt::format("v{}x{}ei{}.v", mjop == 0x7 ? "lo" : "so",
  ...
  if (mjop == 0x7) {
    return rvv_insn_desc{
        .id = RVV_INSN_TYPE::RVV_LD_OINDEXED,
        .opcode = opcode,
        .implicit_src_cnt = 5,
        .implicit_dest_cnt = 1,
        .opr_cnt = 6,
        ...
        .mnemonic = mnemonic,
        .operand_str = opstr,
        .imp_srcs = {rs1, vs2, vm, nf, riscv_sysreg::RISCV_V_SYSREG_VTYPE},
        .imp_dests = {vd_or_vs3},
    ...
    return rvv_insn_desc{
        .id = RVV_INSN_TYPE::RVV_ST_OINDEXED,
        .opcode = opcode,
        .implicit_src_cnt = 6,
        .implicit_dest_cnt = 0,
        .opr_cnt = 6,
